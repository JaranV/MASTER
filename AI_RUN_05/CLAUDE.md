...
For verification, prefer short-lived commands (`./mvnw compile`, `./mvnw package`, `./mvnw test`, `npm run build`). Do not start long-running servers (`./mvnw spring-boot:run`, `npm run dev`, `stripe listen`) unless the user explicitly asks to interactively test. If a long-running process is started, stop it before the turn ends.
